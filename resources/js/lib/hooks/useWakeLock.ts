// credits: https://github.com/dlitsman/react-use-wake-lock
...
export default function useWakeLock(options?: Options): UseWakeLockResult {
    ...
    const onError = useCallback<NonNullable<Options["onError"]>>(
        (err, type) => {
    ...
    const request = useCallback(async () => {
        if (!isSupported) {
            console.warn("WakeLock is not supported by the browser");
            return;
        }

        if (wakeLockInFlight.current === true) {
            // console.warn("WakeLock request is in progress. noop");
            return;
        }

        if (lock != null && lock.released === false) {
            // console.warn("Already have a lock. noop");
            return;
        }
    ...
    const release = useCallback(async () => {
        ...
        if (lock == null) {
            // console.warn("Trying to release lock without having one: noop");
            return;
        }
